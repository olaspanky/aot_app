...
import {useJsApiLoader, GoogleMap, Marker, Autocomplete, DirectionsRenderer, Loader } from "@react-google-maps/api"
...
const [bookOrderMutation, { isError }] = useBookOrderMutation();
  ...
  estimated_time: pickupDetails.estimated_time,
      ...
      console.log("result is", result.error.data.message);
...
const handleMapData = (data) => {
  ...
  const isDataComplete = () => {
    ...
    const { recipient_name, recipient_phone_number, recipient_address } = deliveryDetails;
    return (
      sender_name !== '' &&
      sender_phone_number !== '' &&
      sender_address !== '' &&
...
<Map recipientAddress={deliveryDetails.recipient_address} senderAddress={pickupDetails.sender_address}         onMapDataChange={handleMapDataChange}
/>
  </div>
...
<div className='flex flex-col gap-3'>
      <div className='w-full  grid grid-cols-12  space py-2 px-5 rounded-sm around bg-[#d4d4d4]'>
        ...
        <div
          ...
          onClick={() => {
            setPickupDropdownVisible(prevPickupDropdownVisible => {
      ...
      {pickupDropdownVisible && (
          ...
          {/* Render editable pickup details form */}
            <form>
              ...
              <label className='col-span-3 text-sm font-semibold'>Name:</label>
              <input
                type="text"
                value={pickupDetails.sender_name}
                onChange={(e) => setPickupDetails({ ...pickupDetails, sender_name: e.target.value })}
              ...
              <input
                type="tel"
                value={pickupDetails.sender_phone_number}
                onChange={(e) => setPickupDetails({ ...pickupDetails, sender_phone_number: e.target.value })}
                className='rounded-md col-span-9 p-2'
                placeholder='Sender Phone no.'
              />
              </div>
              <div className='grid grid-cols-12  w-full gap-2 py-3'>
              <label className="col-span-3 text-sm font-semibold">Category:</label>
              <input
                type="text"
                value={pickupDetails.item_category}
                onChange={(e) => setPickupDetails({ ...pickupDetails, item_category: e.target.value })}
                className='rounded-md col-span-9 p-2'
                placeholder='Item Category'
              />
              </div>
              <div className='grid grid-cols-12  w-full gap-2 py-3'>
              <label className="col-span-3 text-sm font-semibold">Quantity:</label>
              <input
                type="number"
                value={pickupDetails.quantity}
                onChange={(e) => setPickupDetails({ ...pickupDetails, quantity: e.target.value })}
                className='rounded-md col-span-9 p-2'
                placeholder='Quantity'
              />
              </div>
              <div className='grid grid-cols-12  w-full gap-2 py-3'>
              <label className="col-span-3 text-sm font-semibold">Sender Address:</label>
              <div className='col-span-9'>

              <Autocomplete cla  onLoad={handleOriginAutocompleteLoad} onPlaceChanged={() => handleOriginPlaceChanged(originAutocomplete)}>
  <input
    type="text"
    value={pickupDetails.sender_address}
    onChange={(e) => setPickupDetails({ ...pickupDetails, sender_address: e.target.value })}
...
<h1>estimated Time: {duration}</h1>
      ...
      <div className='w-full px-5 grid grid-cols-12 flex space py-2 rounded-sm around bg-[#d4d4d4]'>
        ...
        <div
          className='col-span-11 flex justify-between font-bold'
          onClick={() => {
            setDeliveryDropdownVisible(prevDeliveryDropdownVisible => {
        ...
        >
          ...
          <p className='text-[24px] text-[#FF7D00'>{deliveryDropdownVisible ? '-' : '+'}</p>
        </div>
      </div>
          ...
          <div className='bg-[#d4d4d4] px-5 py-2'>
            ...
            <form>
              ...
              <input
                ...
                onChange={(e) => setDeliveryDetails({ ...deliveryDetails, recipient_name: e.target.value })}
                className='rounded-md col-span-9 p-2'
              ...
              <input
                ...
                onChange={(e) => setDeliveryDetails({ ...deliveryDetails, recipient_phone_number: e.target.value })}
                className='rounded-md col-span-9 p-2'
                placeholder='Recipient Phone no.'
              />
              </div>
              <div className='grid grid-cols-12  w-full gap-2 py-3'>
              <label className='col-span-3 text-sm font-semibold'>Recipient Address:</label>
              <div className='col-span-9'>
              <Autocomplete onLoad={handleDestinationAutocompleteLoad} onPlaceChanged={() => handleDestinationPlaceChanged(destinationAutocomplete)}>
  <input
    type="text"
    value={deliveryDetails.recipient_address}
    onChange={(e) => setDeliveryDetails({ ...deliveryDetails, recipient_address: e.target.value })}
  ...
  />
              ...
              {paymentResponse && (
        <div className="alert">
          <span className="closebtn" onClick={() => setPaymentResponse('')}>&times;</span>
          <strong>Payment Response:</strong> {paymentResponse}
        </div>
      )}
...
<button
    ...
    >
      <div className='col-span-1 animate-slide'>
        <Image src={triple} width={32} height={32} alt='Triple' />
      </div>
